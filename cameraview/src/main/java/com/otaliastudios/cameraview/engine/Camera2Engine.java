...
public class Camera2Engine extends CameraBaseEngine implements ImageReader.OnImageAvailableListener, ActionHolder {
  ...
  @NonNull
  private <T> T readCharacteristic(
  ...
  private void applyRepeatingRequestBuilder(boolean checkStarted, int errorReason) {
    if ((getState() == CameraState.PREVIEW && !isChangingState()) || !checkStarted) {
      try {
        mSession.setRepeatingRequest(mRepeatingRequestBuilder.build(), mRepeatingRequestCallback, null);
  ...
  protected Task<Void> onStartBind() {
    ...
    if (hasFrameProcessors()) {
      ...
      mFrameProcessingReader.setOnImageAvailableListener(
          this,
          null
      );
  ...
  protected Task<Void> onStopEngine() {
    try {
      ...
      // 12:33:33.206  1512  3616 E Camera3-Device: Camera 0: waitUntilDrainedLocked: Error waiting for HAL to drain:
      // Connection timed out (-110)[0m
  ...
  protected void onTakePictureSnapshot(
      @NonNull final PictureResult.Stub stub,
  ...
  protected void applyDefaultFocus(@NonNull CaptureRequest.Builder builder) {
    int[] modesArray = readCharacteristic(
  ...
  public void setFlash(@NonNull final Flash flash) {
    ...
    mFlash = flash;
    mFlashTask = getOrchestrator().scheduleStateful(
        "flash (" + flash + ")",
        CameraState.ENGINE,
        new Runnable() {
          @Override
  ...
  public void setLocation(@Nullable Location location) {
    ...
    mLocation = location;
    mLocationTask = getOrchestrator().scheduleStateful(
        "location",
        CameraState.ENGINE,
        new Runnable() {
          @Override
  ...
  @SuppressWarnings("WeakerAccess")
  protected boolean applyLocation(
      @NonNull CaptureRequest.Builder builder,
  ...
  @SuppressWarnings("WeakerAccess")
  protected boolean applyWhiteBalance(
      @NonNull CaptureRequest.Builder builder,
  ...
  public void setHdr(@NonNull Hdr hdr) {
    final Hdr old = mHdr;
    mHdr = hdr;
    mHdrTask = getOrchestrator().scheduleStateful(
        "hdr (" + hdr + ")",
        CameraState.ENGINE,
        new Runnable() {
          @Override
  ...
  private Rect getZoomRect(float zoomLevel, float maxDigitalZoom) {
    Rect activeRect = readCharacteristic(
  ...
  @Override
  public void setExposureCorrection(
      final float EVvalue,
  ...
  @SuppressWarnings("WeakerAccess")
  protected boolean applyExposureCorrection(
      @NonNull CaptureRequest.Builder builder,
      float oldEVvalue
  ) {
  ...
  public void onImageAvailable(ImageReader reader) {
    ...
    if (image == null) {
      ...
      //noinspection unchecked
      Frame frame = getFrameManager().getFrame(
          image,
  ...
  public void setFrameProcessingFormat(final int format) {
    ...
    if (mFrameProcessingFormat == 0) {
      mFrameProcessingFormat = FRAME_PROCESSING_FORMAT;
    }
  ...
  public void startAutoFocus(
      @Nullable final Gesture gesture,
  ...
  private void toggleCamera(String camera) {
    if (camera.equals(mCameraId)) {
      return;
    }

    mCameraId = camera;
    getOrchestrator().scheduleStateful("toggle wide camera", CameraState.ENGINE, new Runnable() {
          @Override
          public void run() {
            restart();
          }
